    ...
    for (const field of domain.fields) {
        ...
        if (field.type === 'number') {
            ...
            if (!Number.isFinite(numeric))
                ...
                warnings.push(`${field.label} cannot be negative.`);
        }
    }
    for (const row of state.rows || []) {
        const score = Number(row.score || 0);
        if (score < 0 || score > 10)
            warnings.push(`${row.label} score must stay between 0 and 10.`);
        if (row.approved && !String(row.value || '').trim())
            warnings.push(`${row.label} needs evidence before approval.`);
        if (row.approved && score < 7)
            warnings.push(`${row.label} approval needs a score of 7 or higher.`);
    }
...
export function buildDomainMarkdown(config, domain, state) {
    ...
    if (calc.warnings.length)
        ...
        lines.push('- No domain validation warnings.');
