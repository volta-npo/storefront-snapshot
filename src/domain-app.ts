...
import { createDomainState, calculateDomain, generateDomainArtifacts, buildDomainMarkdown, buildDomainCsv, applyDomainSample, buildSaasSummary, buildSaasWorkflow, buildClientBrief, buildSaasJson, buildProductBacklogCsv } from './domain-core.js';
...
function install(){
  document.querySelector('.grid').insertAdjacentHTML('beforebegin', `<section class="domain-tool domain-${domain.kind} panel" aria-labelledby="domain-title">
    <div class="domain-head"><div><p class="eyebrow">Standalone SaaS workbench</p><h2 id="domain-title">${esc(domain.title)}</h2><p class="muted">${esc(domain.purpose)}</p></div><div class="button-row no-print"><button id="domain-sample" class="secondary">Load Domain Sample</button><button id="domain-export">Export Tool Packet</button><button id="domain-brief" class="secondary">Export Client Brief</button><button id="domain-json" class="secondary">Export SaaS JSON</button><button id="domain-csv" class="secondary">Export Domain CSV</button><button id="domain-backlog" class="secondary">Export Backlog CSV</button></div></div>
